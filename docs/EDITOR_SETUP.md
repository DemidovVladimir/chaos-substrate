...
Hooks ship for Claude Code and Cursor (see [Hooks](#hooks)). All five editors get the same fourteen MCP
...
3. In the editor, confirm the fourteen MCP tools are listed: `chaos_analyze`, `chaos_add`,
   ...
   `chaos_change_plan`, `chaos_components`, `chaos_features`, and `chaos_project`. See the
