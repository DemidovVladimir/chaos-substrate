...
The plugin bundles the skill, the sixteen MCP tools, and the tool-use hooks together.
...
- The sixteen MCP tools are listed: `chaos_analyze`, `chaos_add`, `chaos_stats`, `chaos_query`,
  `chaos_feature_context`, `chaos_impact`, `chaos_write_feature_website`, `chaos_obsidian`, `chaos_refresh`, `chaos_write_storyboard`, `chaos_change_plan`, `chaos_components`, `chaos_features`, `chaos_project`, `chaos_help`, `chaos_clean`.
...
- README → [MCP Tools](../README.md#mcp-tools) — the sixteen-tool reference.
