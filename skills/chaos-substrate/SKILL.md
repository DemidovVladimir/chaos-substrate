...
1. If `CHAOS_SUBSTRATE_HOME` is set, use `$CHAOS_SUBSTRATE_HOME/scripts/chaos-agent`.
2. Else if `chaos-agent` is on `PATH`, use `chaos-agent`.
...
In command examples, call this resolved wrapper as `$CHAOS_AGENT`. Always pass the target project as
an absolute path or as `"$PWD"` after confirming the shell is in the target project.

Natural language mapping:

- "Go through this project and create sufficient index and explanation" -> `$CHAOS_AGENT init <repo-path>`
...
is set for `doctor`, `init`, and `update`, run migrations, analyze the project, regenerate the
Obsidian vault, and generate dark feature-context websites when requested. `refresh` regenerates the
