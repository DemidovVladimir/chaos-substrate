...
- Agent surfaces are CLI, `scripts/chaos-agent`, static graph/Obsidian exports, generated
...
- "Add this to Claude Code" or "use with Claude Cowork" -> `scripts/chaos-agent claude-code-add local <project-path>` or `scripts/chaos-agent claude-code-add project <project-path>`. The path is the target Claude Code project where config should be applied.
...
already available. If they are not, run `scripts/chaos-agent doctor` or `scripts/chaos-agent init`
first.
