...
Use it to create and query a persistent knowledge base for Rust, Solidity, TypeScript, JavaScript, Python, and GraphQL repositories, with Markdown/MDX and PDF context. The memory is stored in Postgres + pgvector and survives process restarts.

TypeScript, JavaScript, Python, Solidity, and GraphQL are analysis targets only; they are extracted Rust-side and never run as a separate service.
...
- `chaos_usage`: report who consumes a symbol or surface string (env var, HTTP header, route, GraphQL field, function) across the repo, grouped by top-level subfolder — from the persisted index only (user-surface `env_var`/`http_route`/`cli_command`/`graphql_field` nodes, reverse graph edges, and a literal chunk sweep), never by grepping the target repo. Always writes `docs/features_memory/<slug>-usage.html`. Read-only and embedder-free.
...
- `chaos_stack`: report the tech stack of an already-indexed repository (dependencies by ecosystem, npm scripts, AWS CDK resources, JS/TS configs, the exposed API surface — HTTP routes, SDL-derived GraphQL root fields, CLI commands — and the language breakdown). Always writes `docs/features_memory/stack.html`. Read-only and embedder-free.
...
- `chaos_project`: manage cross-repository projects — create, add repos, index project-level docs (`add_docs`), list, check status, relink. Detects feature→feature cross-repo links (`package_dep`, `abi`, `http_route`, `graphql`) from the persisted index.
- `chaos_feature_story`: tell the cross-repo story of one feature across a project — matches it in every member repo, traverses the cross-repo links, orders the involved features client → backend → contracts, and writes a clickable multi-page site to the project workspace. Returns a compact summary; narrate the returned spine.
